...
4. **Domain.** Point `www.yirehtextiles.com.au` (GoDaddy) at the host and add the
   bare `yirehtextiles.com.au` too — `next.config.ts` 308-redirects it to `www`.
...
Canonical host is **`https://www.yirehtextiles.com.au`**. Every canonical tag,
sitemap URL and JSON-LD `@id` uses it, and the bare apex redirects to it, so
add the site to Search Console as that host.

1. **Add a property.** Prefer a *Domain* property (`yirehtextiles.com.au`) — it
   ...
   - Alternative: a *URL-prefix* property for `https://www.yirehtextiles.com.au/`
...
2. **Submit the sitemap.** Sitemaps → add `https://www.yirehtextiles.com.au/sitemap.xml`.
